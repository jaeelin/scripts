-- hello skid, this is the first open source best script on this game so the fact that you are contemplating skidding this proves that this is the best
-- join our discord server and stop being a skid https://discord.gg/rxpEc3pCMR
...
local tabs = {
	...
	World = main_group:Tab({Name = "World", Image = "rbxassetid://11887653877"}),
...
KnifeAura = sections.combat_left_bottom2:Toggle({
	Name = "Knife Aura",
...
Core.Keybinds.KnifeAura = sections.combat_left_bottom2:Keybind({
...
SetCooldown = sections.combat_right:Toggle({
	Name = "Set Cooldown",
...
Core.Keybinds.SetCooldown = sections.combat_right:Keybind({
	Name = "Set Cooldown Keybind",
	Blacklist = false,
	onBindHeld = function(held)
		if held then
			Core.Features.SetCooldown.Enabled = not Core.Features.SetCooldown.Enabled
			SetCooldown:UpdateState(Core.Features.SetCooldown.Enabled)
		end
	end,
}, "SetCooldownKeybind")

SetThrowSpeed = sections.combat_right:Toggle({
...
Core.Keybinds.SetThrowSpeed = sections.combat_right:Keybind({
	Name = "Set Throw Speed Keybind",
	Blacklist = false,
	onBindHeld = function(held)
		if held then
			Core.Features.SetThrowSpeed.Enabled = not Core.Features.SetThrowSpeed.Enabled
			SetThrowSpeed:UpdateState(Core.Features.SetThrowSpeed.Enabled)
		end
	end,
}, "SetThrowSpeedKeybind")

HitboxExtender = sections.combat_right_bottom:Toggle({
		...
		if enabled then
			Core.Connections.HitboxExtender = Services.RunService.PreRender:Connect(function()
				for _, target in next, Services.Players:GetPlayers() do
					...
					local humanoid_root_part = target:FindFirstChild("HumanoidRootPart")
					if not humanoid_root_part then continue end
					
					humanoid_root_part.Size = Core.Features.HitboxExtender.Size
					humanoid_root_part.Transparency = Core.Features.HitboxExtender.Transparency
				end
			end)
		else
...
sections.combat_right_bottom:Slider({
	Name = "Size",
...
}, "HitboxExtenderTransparency")

Core.Keybinds.combat_right_bottom = sections.combat_right_bottom:Keybind({
	Name = "Hitbox Extender Keybind",
	Blacklist = false,
	onBindHeld = function(held)
		if held then
			Core.Features.HitboxExtender.Enabled = not Core.Features.HitboxExtender.Enabled
			HitboxExtender:UpdateState(Core.Features.HitboxExtender.Enabled)
		end
	end,
}, "HitboxExtenderKeybind")

--[[ WORLD ]]--

CrashAll = sections.world_left:Toggle({
	Name = "Crash All",
	Default = Core.Features.CrashAll.Enabled,
	Callback = function(enabled)
		Core.Features.CrashAll.Enabled = enabled

		if enabled then
			Core.Connections.CrashAll = Services.RunService.PreRender:Connect(function()
				local character = LocalPlayer.Character
				if not character then return end
				...
				local args = {
					humanoid_root_part.Position + Vector3.new(15, 15, 15),
					humanoid_root_part.Position + Vector3.new(15, 15, 15)
				}
				Services.ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("ThrowStart"):FireServer(unpack(args))
				
				local args = {
					humanoid_root_part,
					humanoid_root_part.Position + Vector3.new(15, 15, 15)
				}
				Services.ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("ThrowHit"):FireServer(unpack(args))
			end)
		else
			if Core.Connections.CrashAll then
				Core.Connections.CrashAll:Disconnect()
				Core.Connections.CrashAll = nil
			end
		end
	end,
}, "CrashAll")

Core.Keybinds.CrashAll = sections.world_left:Keybind({
	Name = "Crash All Keybind",
	Blacklist = false,
	onBindHeld = function(held)
		if held then
			Core.Features.CrashAll.Enabled = not Core.Features.CrashAll.Enabled
			CrashAll:UpdateState(Core.Features.CrashAll.Enabled)
		end
	end,
}, "CrashAllKeybind")

AntiCrash = sections.world_left:Toggle({
	Name = "Anti Crash",
	Default = Core.Features.AntiCrash.Enabled,
	Callback = function(enabled)
		Core.Features.AntiCrash.Enabled = enabled

		if enabled then
			Core.Connections.AntiCrash = Services.RunService.PreRender:Connect(function()
				local remotes = Services.ReplicatedStorage:FindFirstChild("Remotes")
				if not remotes then return end
				
				local render_projectile = remotes:FindFirstChild("RenderKnifeProjectile")
				if not render_projectile then return end
				
				render_projectile:Destroy()
			end)
		else
			if Core.Connections.AntiCrash then
				Core.Connections.AntiCrash:Disconnect()
				Core.Connections.AntiCrash = nil
			end
			...
			local render_projectile = remotes:FindFirstChild("RenderKnifeProjectile")
			if not render_projectile then
				local new = Instance.new("RemoteEvent")
				new.Name = "RenderKnifeProjectile"
				new.Parent = remotes
			end
		end
	end,
...
Core.Keybinds.AntiCrash = sections.world_left:Keybind({
	Name = "Anti Crash Keybind",
	Blacklist = false,
	onBindHeld = function(held)
		if held then
			Core.Features.AntiCrash.Enabled = not Core.Features.AntiCrash.Enabled
			AntiCrash:UpdateState(Core.Features.AntiCrash.Enabled)
		end
	end,
}, "AntiCrashKeybind")
...
}, "PlayerESPRemoveHiddenCharacters")
